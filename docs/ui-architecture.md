...
The default rule should keep layout and hit testing based on logical retained rectangles while rendering applies transient alpha, scale, frame selection, or small offsets. `UiScreen` owns frame-time dispatch through `tickUi`, while `UiWidget` owns recursive widget-local tick hooks. `UiWindow` owns logical open/close transition state and progress; visual transition parameters still need to be exposed to rendering. The renderer should draw resolved geometry and optional transform or texture-frame data; it should not own animation policy.
